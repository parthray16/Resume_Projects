...
bool Cache::access(unsigned int address) {
  ...
  unsigned int numbitsIndex = (unsigned int) log2(rows);
  unsigned int numbitsTag = 32 - (numbitsByteSelect + numbitsIndex);
  unsigned int tag = address >> (numbitsByteSelect + numbitsIndex);
  unsigned int index = (address << (numbitsTag)) >> (numbitsTag + numbitsByteSelect);
  if (rows == 1){
    index = 0;
  }
...
void Stats::print() {
  cout << dec
       << "Total number of dynamic instructions: " << dec << instrs << endl
